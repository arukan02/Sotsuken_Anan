...
out_file_name_ue= 'result_ue.csv'

# Create the folder if it doesn't exist
if not os.path.exists(result_file_path):
    os.makedirs(result_file_path)

if not os.path.exists(os.path.join(result_file_path, out_file_name_ue)):
    with open(os.path.join(result_file_path, out_file_name_ue), 'w'):
        pass
...
def run_worflow(my_image, my_model, frame_num):
  ...
  for threshold in [0.9]:
    ...
    with open(result_file_path + '/' + out_file_name_ue , 'a',newline='') as f:
      writer = csv.writer(f)
